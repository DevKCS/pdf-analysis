...
app.post('/makePdf1', async (req, res) => {
    ...
    createlaboratory += `연구소를 설립하여 "2명"의 연구원의 임금을 1년 "35,000,000원"으로 책정했을 경우,<br/>연구원 투입 임금의 25%가 세액공제 된다.\n즉, 35,000,000원 x 2명 x 25% = 17,500,000원의 세액이 절감 된다.<br/>${numberToKorean(Number(pdf1.resultJson['세금']))}원 - 17,500,000원 = ${Number(pdf1.resultJson['세금']) - 17500000 < 0 ? 0 : Number(pdf1.resultJson['세금']) - 17500000}${Number(pdf1.resultJson['세금']) - 17500000 < 0 ? `(${Number(pdf1.resultJson['세금']) - 17500000}원 이월)` : ""}원이다.`
    const table = {
        headers: ['항목', ''],
        rows: [
...
app.post('/makePdf2', async (req, res) => {
    ...
    createlaboratory += `연구소를 설립하여 "2명"의 연구원의 임금을 1년 "35,000,000원"으로 책정했을 경우,<br/>연구원 투입 임금의 25%가 세액공제 된다.\n즉, 35,000,000원 x 2명 x 25% = 17,500,000원의 세액이 절감 된다.<br/>${numberToKorean(Number(pdf2.resultJson['세금']))}원 - 17,500,000원 = ${Number(pdf2.resultJson['세금']) - 17500000 < 0 ? 0 : Number(pdf2.resultJson['세금']) - 17500000}${Number(pdf2.resultJson['세금']) - 17500000 < 0 ? `(${Number(pdf2.resultJson['세금']) - 17500000}원 이월)` : ""}원이다.`
    const table = {
        headers: ['항목', '금액'],
        rows: [
            ['장기차입금', reportData.loanAmount],
